...
const MessageHistory = () => {
    ...
    const data = [
        {
            id: 129,
            content: 'This is another test message. I believe that I can fly and anything I set my mind to do, I can accomplish.',
        ...
        <div>
            ...
            <Col span={24} className="home-container">
                <Space direction='vertical'>
                    {data.map((item) => (
                        <Card hoverable={isFuture(item.scheduled_at)} className={isFuture(item.scheduled_at) ? 'future-message' : ''}>
                            <Typography.Title level={5}>{item.content}</Typography.Title>
                            <Typography.Text type="secondary">{renderTimestampText(item.scheduled_at)}</Typography.Text>
                        </Card>
                    ))}
                </Space>
            </Col>
